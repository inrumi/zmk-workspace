...
5. **Git Branch & Commit Safety:** Never commit directly on `main` branch. Only create commits if working on a branch that is not `main` AND explicitly requested/authorized by the user; otherwise keep all changes uncommitted and visible. Do not flash hardware (the user tests physical firmware flashes independently).
